...
void store_in_map(int data, char *arg, machine_t *machine, champion_t *champion)
{
    int pc = get_val(arg, true) % IDX_MOD;

    add_pc(&pc, champion->process->pc);
    print_int_in_map(pc, machine, data, champion);
}
...
champion_t *champion)
{
    ...
    if (instruct->mask[1] == INDIRECT)
        store_in_map(data, instruct->arg[1], machine, champion);
    else
...
{
    do_st(champion->process->instruct, champion->process, machine, champion);
}
