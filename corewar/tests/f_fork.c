...
Test(Corewar, create_process)
{
    ...
    f_fork(&machine, machine.champion[0]);
...
Test(Corewar, create_process_backward)
{
    machine_t machine;
...
Test(Corewar, create_process_backward_mem_circle)
{
    machine_t machine;
...
Test(Corewar, create_process_with_pc_idx_mod)
{
    machine_t machine;
    ...
    cr_expect_eq(machine.champion[0]->process->pc, 8);
    cr_assert(machine.champion[0]->process->next);
    cr_expect_eq(machine.champion[0]->process->next->pc, 5);
}
